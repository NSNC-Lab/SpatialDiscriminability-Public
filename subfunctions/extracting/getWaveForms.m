...
[b,a] = butter(3, [300 5000] / (24414.0625/2) , 'bandpass');
for ch = 1:gwfparams.nCh
    filtered_raw(ch,:) = filtfilt(b,a,double(mmf.Data.x(ch,:)));
end
...
for curUnitInd = 1:numUnits
    ...
    curSpikeTimes(curSpikeTimes+gwfparams.wfWin(end) > size(filtered_raw,2)) = [];  %%% noconjio
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        tmpWf = filtered_raw(1:gwfparams.nCh,spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end));
