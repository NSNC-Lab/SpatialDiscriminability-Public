function [perf_file,goodchans,perf_struct] = calc_performance_RISPIKE(Spks_clean,Spks_masked,folder,...
...
for ch = 1:chlim
    ...
    for i = 1:4 % stimuli loc
        ...
        if k > 1
            ...
            distMat = STS.RateIndependentSPIKEdistanceMatrix(0,3);
        ...
        for j = 1:4 % masker loc
            ...
            if k2 > 1
                ...
                STS = SpikeTrainSet(input,0,3);
                distMat_masked = STS.RateIndependentSPIKEdistanceMatrix(0,3);
...
if saveFile
    perf_file = [folder,'/',subject,'-',num2str(power),'mW_',num2str(TMR),...
        'dBTMR_performance_RI_SPIKE',type,'.mat'];
